...
export class ComixController {
    ...
    public async getComixByName(
        @Query() param: { name?: string, author?: string, painter?:string, genres?: string[], bookbider: string , side:string},
        @Res() res: Response,
    ): Promise<Response> {
        try {
            const { name, author,painter, genres, bookbider, side} = param;
            const comixes = await this.comixService.getByParams(name, author,painter, genres, bookbider, side);
